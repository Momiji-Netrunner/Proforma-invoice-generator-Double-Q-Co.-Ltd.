...
def smart_linebreak_specs(text):
    keywords = [
        ...
        "Automatic transmission,", "King Pin Setting", 
        "Cargo", "Color:", "Load:", "Fuel", "Rear Axle", "With", "Volume of the compartment", 
        "Axle: FUWA", "Fifth Wheel Height", "Axle Space",
        "Tare weight", "Payload Capacity", "Main beam", 
        "Flange and Web", "Body(mm)", "Leaf Spring(mm)",
        "Hydraulic System(mm)", "Landing Gear (Supports)",
        "Toolbox:", "Spare Tire Carrier:", "Rim:", "Axles: Factory",
        "Traction pin", "Brake chamber", "Suspension:",
        "ABS:None", "Kingpin:", "Braking System:", "2 stage 3meter full free lift mast", "QUANCHAI 4C6 engine，",
        "Automatic transmission，", "1220mm fork", "Engine : Isuzu",
        "Rated Power", "Operating weight", "Bucket capacity", "A/C cab",
        "Rated power", "Gross weight:", "Dumping height", 
        "Maximum breakout force", "Axle：FUWA", "suspension:", "Tire：",
        "Cat Ladder:", "Tool box:", "Spare Wheel Carrier:", "Lamp:",
...
def populate_notice_row(row, model, item_no, delivery_days, warranty_hours, warranty_months, unit):
    ...
    if warranty_months == 0 and warranty_hours == 0:
        warranty_text = "Others"
    else:
        warranty_text = f"Within {warranty_months} months or {warranty_value} {unit}, whichever comes first"
...
def get_income_advance_base(model_text, currency):
    ...
    if "3 AXLE" in model_text:
        return 450 if currency == "USD" else 1650000
    if "60 TONS LOW BED SEMI TRAILER" in model_text:
        return 500 if currency == "USD" else 1650000
...
if os.path.exists(rindex_image_path):
    if Image is not None:
        try:
            original_rindex_image = Image.open(rindex_image_path)
        except Exception:
            original_rindex_image = None
    else:
        try:
            original_rindex_image = tk.PhotoImage(file=rindex_image_path)
        except Exception:
            original_rindex_image = None

if original_rindex_image is not None:
    rindex_image_label = ttk.Label(info_frame)
    rindex_image_label.grid(row=0, column=3, rowspan=9, sticky="nsew", padx=(8, 0), pady=(0,0))

    if Image is None:
        rindex_image_label.configure(image=original_rindex_image)
        rindex_image_label.image = original_rindex_image
    else:
        def update_rindex_image(_event=None):
            if original_rindex_image is None:
                return

            width = rindex_image_label.winfo_width()
            height = rindex_image_label.winfo_height()
            if width <= 1 or height <= 1:
                return

            width -= 8
            height -= 8
            img = original_rindex_image.copy()
            img.thumbnail((width, height), Image.LANCZOS)
            rindex_image_label.image = ImageTk.PhotoImage(img)
            rindex_image_label.configure(image=rindex_image_label.image)

        info_frame.bind("<Configure>", update_rindex_image)
        root.after(150, update_rindex_image)
...
def set_generation_status(text, color=None):
...
def generate_proforma_thread():
    ...
    data = {
        ...
        "pi": pi_var.get().strip(),
        "sales": sales_var.get().strip(),
        "client": client_var.get().strip(),
        "tel": tel_var.get().strip(),
        "email": email_var.get().strip() or "henock@doubleq.co.ug",
        "issue_date": issue_date.get_date(),
        "valid_date": valid_date.get_date(),
        "currency": get_currency_prefix(currency_var.get()),
        "brand": brand_var.get(),
        "extra_line_enabled": extra_line_enabled_var.get(),
        "extra_line_label": extra_line_label_var.get().strip(),
        "extra_line_amount": extra_line_amount_var.get(),
    }

    generate_button.state(["disabled"])
    set_generation_status("Generating document...")

    worker = threading.Thread(target=generate_proforma_worker, args=(data,), daemon=True)
    worker.start()


def handle_generate_result(result, error):
    generate_button.state(["!disabled"])
    if error:
        set_generation_status("Generation failed.", "red")
        messagebox.showerror("Error", f"Failed to generate document:\n{str(error)}")
        return

    set_generation_status("Generation completed.", "green")
    messagebox.showinfo(
        "Success",
...
def generate_proforma_worker(data):
    try:
        ...
        replacements = {
            "DQME2025100202M": data["pi"],
            "Henock Sean": data["sales"],
            "Aahil Ventures LTD": data["client"],
            "+256-788801299": data["tel"],
            "henock@doubleq.co.ug": data["email"],
            "2025-OCTOBER-02": data["issue_date"].strftime("%Y-%B-%d").upper(),
            "2025-NOVEMBER-01": data["valid_date"].strftime("%Y-%B-%d").upper(),
        }
        ...
        styled_values = [
            (data["pi"], True, True, 11),
            (data["sales"], False, False, None),
            (data["client"], False, False, None),
            (data["tel"], False, False, None),
            (data["email"], False, False, None),
            (data["issue_date"].strftime("%Y-%B-%d").upper(), False, False, None),
            (data["valid_date"].strftime("%Y-%B-%d").upper(), False, False, None),
        ]
        ...
        if data["brand"] == "Sinotruk":
            income_tax_amount = sum(
                get_income_advance_base(product["model"], currency) * product["qty"]
                for product in data["products"]
                if get_income_advance_base(product["model"], currency)
            )
            income_tax_amount = round(income_tax_amount, 2)
            if income_tax_amount > 0 and data["extra_line_enabled"]:
        ...
        if data["brand"] != "Sinotruk" and data["extra_line_enabled"] and data["extra_line_label"]:
            extra_amount = round(float(data["extra_line_amount"]), 2)
            if extra_amount:
        ...
        output_name = f"{data['pi']}.docx"
        output_dir = os.path.join(BASE_DIR, "output")
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, output_name)
        doc.save(output_path)

        root.after(0, handle_generate_result, {"output_name": output_name, "output_dir": output_dir}, None)
    except Exception as e:
        root.after(0, handle_generate_result, None, e)
...
generate_button = ttk.Button(content_frame, text="Generate", command=generate_proforma_thread, style="Accent.TButton")
generate_button.pack(pady=6)
status_label = ttk.Label(content_frame, textvariable=progress_var, foreground="black")
status_label.pack()
