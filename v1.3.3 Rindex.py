...
def get_income_advance_base(model_text, currency):
    ...
    if "5O T LOW BED SEMI-TRAILER" in model_text:
        return 500 if currency == "USD" else 1650000
    if "50 T LOW BED SEMI-TRAILER" in model_text:
        return 500 if currency == "USD" else 1650000
...
ttk.Checkbutton(add_frame, text="Extra Line", variable=extra_line_enabled_var).grid(row=6, column=0, columnspan=2, sticky="w", padx=5, pady=8)
...
def generate_proforma():
    ...
    try:
        ...
        if brand_var.get() == "Sinotruk":
            ...
            if income_tax_amount > 0 and extra_line_enabled_var.get():
                ...
                set_cell_text(
                    ...
                    f"{currency} {format_currency(extra_line_amount_var.get())}",
                    bold=True,
                    underline=True,
                    font_name="Times New Roman",
                    font_size=10,
